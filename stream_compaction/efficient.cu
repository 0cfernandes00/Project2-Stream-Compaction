...
namespace StreamCompaction {
    namespace Efficient {
        ...
        __global__ void kernDownSweepEfficientScan(int n, int d, int* odata) {
			...
			int threadsNeeded = n >> (d + 1);
			if (k >= threadsNeeded) return;


            int baseIdx = stride * k;
            int leftIdx = baseIdx + (1 << d) - 1;
            int rightIdx = baseIdx + stride - 1;
        ...
        __global__ void kernUpSweepEfficientScan(int n, int d, int* odata) {
            ...
            int threadsNeeded = n >> (d + 1);
            if (index >= threadsNeeded) return;

            int baseIdx = stride * index;
            int leftIdx = baseIdx + (1 << d) - 1;
            int rightIdx = baseIdx + stride - 1;
        ...
        void scan(int n, int *odata, const int *idata) {
            ...
            if (paddedN > n) {
                cudaMemset(d_data + n, 0, (paddedN - n) * sizeof(int));
            }
            ...
            for (int d = 0; d < logceilN; ++d) {
                int threadsNeeded = paddedN >> (d + 1);
                if (threadsNeeded == 0) break;

                int numBlocksLevel = (threadsNeeded + blockSize - 1) / blockSize;

                kernUpSweepEfficientScan << <numBlocksLevel, blockSize >> > (paddedN, d, d_data);
            ...
            for (int d = logceilN - 1; d > -1; --d) {
                ...
                kernDownSweepEfficientScan << <numBlocksLevel, blockSize >> > (paddedN, d, d_data);
